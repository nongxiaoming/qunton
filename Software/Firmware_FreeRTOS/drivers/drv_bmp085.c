...
/* Private methods */
static int32_t BMP085_Read(uint8_t address, uint8_t *buffer, uint8_t len);
static int32_t BMP085_WriteCommand(uint8_t address, uint8_t buffer);
static void BMP085_Task(void *parameters);
 ...
 */
static struct bmp085_dev *BMP085_alloc(void)
{
   struct bmp085_dev *bmp085_dev;

	bmp085_dev = (struct bmp085_dev *)OS_Malloc(sizeof(*bmp085_dev));
	if (!bmp085_dev) return (NULL);

	bmp085_dev->queue = OS_QueueCreate(1, sizeof(struct sensor_baro_data));
	if (bmp085_dev->queue == NULL) {
	  OS_Free(bmp085_dev);
		return NULL;
	}

	bmp085_dev->busy = Semaphore_Create();

	bmp085_dev->magic = BMP085_DEV_MAGIC;
 ...
 */
static int32_t BMP085_Validate(struct bmp085_dev *dev)
{
	if (dev == NULL)
		return -1;
	if (dev->i2c_id == 0)
 ...
 */
int32_t BMP085_Init(struct bmp085_cfg *cfg, int32_t i2c_device)
{
	dev = (struct bmp085_dev *) BMP085_alloc();
	if (dev == NULL)
		return -1;
	...
	uint8_t data[22];
	if (BMP085_Read(BMP085_CALIB_ADDR, data, 2) != 0)
		return -2;
	...
	if (BMP085_Read(BMP085_CALIB_ADDR, data, 22) != 0)
		return -2;
	/* Parameters AC1-AC6 */
	dev->AC1 = (data[0] << 8) | data[1];
	...
	dev->task = OS_TaskCreate(
		BMP085_Task, "pios_bmp085", BMP085_TASK_STACK, NULL, BMP085_TASK_PRIORITY);
	if (dev->task == NULL)
		return -3;
 ...
 */
static int32_t BMP085_ClaimDevice(void)
{
  OS_Assert(PIOS_BMP085_Validate(dev) == 0);

	return Semaphore_Take(dev->busy, SEMAPHORE_TIMEOUT_MAX) == true ? 0 : 1;
}
 ...
 * \return 0 if no error
 */
static int32_t BMP085_ReleaseDevice(void)
{
	OS_Assert(BMP085_Validate(dev) == 0);

	return Semaphore_Give(dev->busy) == true ? 0 : 1;
}
...
*/
static int32_tBMP085_StartADC(enum conversion_type type)
{
	if (BMP085_Validate(dev) != 0)
		return -1;

	/* Start the conversion */
	switch(type) {
	case TEMPERATURE_CONV:
		while (BMP085_WriteCommand(BMP085_CTRL_ADDR, BMP085_TEMP_ADDR) != 0)
			continue;
		break;
	case PRESSURE_CONV:
		while (BMP085_WriteCommand(BMP085_CTRL_ADDR, BMP085_PRES_ADDR + (dev->oversampling << 6)) != 0)
			continue;
		break;
	default:
		return -1;
 ...
 */
static int32_t BMP085_GetDelay() {
	if (BMP085_Validate(dev) != 0)
		return 100;
...
*/
static int32_t BMP085_ReadADC(void)
{
	if (BMP085_Validate(dev) != 0)
		return -1;
	...
	if (dev->current_conversion_type == TEMPERATURE_CONV) {
		...
		if (BMP085_Read(BMP085_ADC_MSB, data, 2) != 0)
			return -1;
		...
		if (BMP085_Read(BMP085_ADC_MSB, data, 3) != 0)
			return -1;
...
int32_t BMP085_Test()
{
	...
	BMP085_StartADC(TEMPERATURE_CONV);
	DELAY_WaitmS(5);
	BMP085_ReadADC();
	BMP085_ReleaseDevice();
	if (cur_value == dev->temperature_unscaled)
		return -1;

	BMP085_ClaimDevice();
	cur_value = dev->pressure_unscaled;
	BMP085_StartADC(PRESSURE_CONV);
	DELAY_WaitmS(26);
	BMP085_ReadADC();
	BMP085_ReleaseDevice();
...
static void BMP085_Task(void *parameters)
{
	...
	while (1) {
		...
		if(temp_press_interleave_count <= 0)
		{
			...
			BMP085_StartADC(TEMPERATURE_CONV);
			Thread_Sleep(5);
			read_adc_result = BMP085_ReadADC();
			BMP085_ReleaseDevice();
		...
		if (read_adc_result == 0) {
			OS_QueueSend(dev->queue, (void*)&data, 0);
		}
	}
}
