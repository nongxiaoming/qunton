...
//! Private functions
static struct adxl345_dev * ADXL345_alloc(void);
static int32_t ADXL345_Validate(struct adxl345_dev * dev);
static int32_t ADXL345_ClaimBus(void);
static int32_t ADXL345_ReleaseBus(void);
static int32_t ADXL345_FifoDepth(uint8_t depth);
...
static struct adxl345_dev * ADXL345_alloc(void)
{
	...
	adxl345_dev = (struct adxl345_dev *)OS_Malloc(sizeof(*adxl345_dev));
 ...
 */
static int32_t ADXL345_ClaimBus()
{
	if(ADXL345_Validate(dev) != 0)
		return -1;
 ...
 */
static int32_t ADXL345_ReleaseBus()
{
	if(ADXL345_Validate(dev) != 0)
		return -1;
	...
	if(SPI_ReleaseBus(dev->spi_id) != 0)
		return -2;
...
int32_t ADXL345_SelectRate(uint8_t rate)
{
	...
	if(ADXL345_ClaimBus() != 0)
		return -2;

	uint8_t out[2] = {ADXL_RATE_ADDR, rate & 0x0F};
	if(SPI_TransferBlock(dev->spi_id,out,NULL,sizeof(out),NULL) < 0) {
		ADXL345_ReleaseBus();
		return -3;
	}
 ...
 */
static int32_t ADXL345_SetMeasure(uint8_t enable)
{
	if(ADXL345_Validate(dev) != 0)
		return -1;
...
int32_t ADXL345_Init(uint32_t spi_id, uint32_t slave_num)
{
	...
	ADXL345_ReleaseBus();
	ADXL345_SelectRate(ADXL_RATE_3200);
	ADXL345_SetRange(ADXL_RANGE_8G);
	ADXL345_FifoDepth(16);
	ADXL345_SetMeasure(1);
...
int32_t ADXL345_Test(void)
{
	...
	if(SPI_TransferBlock(dev->spi_id,&buf[0],&rec[0],sizeof(buf),NULL) < 0) {
		ADXL345_ReleaseBus();
		return -3;
	}
...
uint8_t ADXL345_Read(struct adxl345_data * data)
{
	...
	if(SPI_TransferBlock(dev->spi_id,&buf[0],&rec[0],9,NULL) < 0) {
		ADXL345_ReleaseBus();
		return -3;
	}
